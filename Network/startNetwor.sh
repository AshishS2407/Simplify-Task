...
peer lifecycle chaincode package basic.tar.gz --path ${PWD}/../Chaincode/ --lang node --label basic_1.0
sleep 1
...
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --channelID $CHANNEL_NAME --name Assets --version 1.0 --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
...
peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name Assets --version 1.0 --sequence 1  --cafile $ORDERER_CA --output json

peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --channelID $CHANNEL_NAME --name Assets --version 1.0 --sequence 1 --tls --cafile $ORDERER_CA --peerAddresses localhost:7051 --tlsRootCertFiles $ORG1_PEER_TLSROOTCERT --peerAddresses localhost:9051 --tlsRootCertFiles $ORG2_PEER_TLSROOTCERT

sleep 1


peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name Assets --cafile $ORDERER_CA



#need to export this export FABRIC_CA_CLIENT_HOME="${PWD}/organizations/peerOrganizations/org1.example.com"
